...
class WikiText_Korean(WikiText):
    def load_raw_dataset(self) -> Dataset:
        dataset = load_dataset("lcw99/wikipedia-korean-20221001")["train"]
        if self.split_str == "test":
            ...
            raise ValueError(
                "Wikitext Korean dataset currently only supports `test` and `train` split"
            )
